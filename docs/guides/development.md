...
cargo run -p vl -- dictation start --mode vad-gated --probe-secs 2 --max-segment-secs 30 --duration-seconds 10 --language auto
...
- `vl dictation list` returns the JSON array from `GET /v1/sessions`. The daemon keeps every `CaptureSession` it has seen since startup (including completed and failed ones) sorted by `created_at_millis`, so an operator who lost a `session_id` printed by an earlier `dictation start` can recover it without restarting the daemon.
- `vl dictation start/stop` uses the daemon UDS client, so the CLI exercises the same control plane hotkey and UI integrations use. `--mode {one-shot,fixed,vad-gated}` selects the segmentation strategy; `fixed` requires `--segment-secs`, `vad-gated` requires `--probe-secs` and `--max-segment-secs` (with an optional `--silence-gap-probes`, default 1). Omitting `--mode` keeps the pre-Phase-3 one-shot behavior. Pass `--duration-seconds N` to have the CLI auto-stop after `N` seconds and print the final `DictationCaptureResult` in one call — useful for smoke / benchmark loops where managing a session id by hand is overhead.
