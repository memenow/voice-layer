...
- Documentation layout: every page under `docs/` ships as both Markdown (the authoritative source the guard tests scan) and a generated HTML mirror for operator-facing reading. After editing any `docs/**/*.md` (or the partials under `docs/assets/`), re-run `scripts/build-docs-html.sh` so the matching `.html` stays in sync. `docs/index.html` is hand-written and is not regenerated by the script.
...
This project is indexed by GitNexus as **voice-layer** (2752 symbols, 5781 relationships, 244 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
