...
//!
//! A single-window iced application that probes the daemon, exposes
//! start/stop buttons for a dictation session, and shows the resulting
//! transcript. Global shortcuts come from the platform backend (XDG
//! `GlobalShortcuts` portal on Linux, Carbon hotkeys on macOS); otherwise
//! the app falls back to an in-window F9 hotkey.
//!
//! Out of scope for this shell: tray, settings center, auto-injection into
//! external apps, multi-window, themes.
...
use hotkeys::{HotkeyProbe, SHORTCUT_TOGGLE};
use overlay::{
    DaemonStatus, HotkeyStatus, SessionStage, SharedError, State, render_daemon_status,
    render_session_stage,
};
use uuid::Uuid;
use voicelayer_client::Client;
use voicelayer_core::{
    CaptureSession, DictationCaptureResult, HealthResponse, SegmentationMode,
...
impl App {
    fn boot() -> (Self, Task<Message>) {
        ...
        let startup = Task::batch(vec![
            probe_health_task(app.socket_path.clone()),
            Task::perform(hotkeys::probe(), Message::HotkeyProbed),
        ]);
        (app, startup)
    }
    ...
    fn update(&mut self, message: Message) -> Task<Message> {
        match message {
            ...
            Message::HotkeyProbed(HotkeyProbe::Available(backend)) => {
            ...
            Message::HotkeyProbed(HotkeyProbe::Unavailable(reason)) => {
                self.state.hotkey = HotkeyStatus {
                    available: false,
                    backend: None,
                    detail: Some(reason),
            ...
            Message::SessionStopped(Ok(result)) => {
                // The daemon only returns Ok for completed captures; any
                // failure arrives as Err with problem details.
    ...
    fn view(&self) -> Element<'_, Message> {
        ...
        let hotkey_line: Element<'_, Message> = if self.state.hotkey.available {
            text(format!(
                "Global shortcut active ({}, F9 toggles dictation)",
                self.state.hotkey.backend.as_deref().unwrap_or("unknown")
            ))
            .size(12)
            .into()
        } else {
    ...
    fn subscription(&self) -> Subscription<Message> {
        ...
        Subscription::batch(vec![keys, Subscription::run(hotkey_stream)])
    }
}

/// Best-effort stream for backend-activated hotkeys. The subscription starts
/// a tokio task that opens the platform hotkey backend and forwards each
/// activation as `Message::HotkeyReceived`. When the backend is unavailable
/// the task exits immediately and the stream stays silent.
fn hotkey_stream() -> impl iced::futures::Stream<Item = Message> {
    stream::channel(8, async |mut output: futures_mpsc::Sender<Message>| {
        ...
        let registration = tokio::spawn(async move {
            if let Err(error) = hotkeys::run_listener(tx).await {
                tracing::warn!(error = %error, "hotkey listener returned error");
            }
        });
...
async fn start_session_request(socket_path: PathBuf) -> Result<CaptureSession, SharedError> {
    ...
    Client::new(socket_path)
        .post("/v1/sessions/dictation", &payload)
        .await
        .map_err(|error| Arc::new(error.to_string()))
}
...
mod tests {
    ...
    /// Serializes env mutation across every test in this module; cargo runs
    /// unit tests concurrently and Rust 2024 makes `set_var` unsafe.
    ...
    fn resolve_daemon_binary_honors_explicit_override() {
        ...
        let _bin_guard = EnvGuard::capture("VOICELAYER_DAEMON_BIN");
